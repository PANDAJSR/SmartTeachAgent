...
async function testMcpHttpConnection(payload?: {
  name?: string;
  url?: string;
  headers?: Record<string, string>;
  ...
  try {
    ...
    if (response.ok) {
      return {
        ...
        reachable: true,
        status,
        message: `${serverName} 连接成功（HTTP ${status}）`,
      };
    }
    return {
      ok: false,
      reachable: true,
      status,
      message: `${serverName} 可达，但握手失败（HTTP ${status}）`,
    };
  } catch (error) {
...
async function runClaudeChat(
  ...
  try {
    ...
    if (sdkMessage.subtype === "success") {
      ...
      logInfo(`[${debugTag}] Claude result 失败详情`, {
        ...
        stopReason:
          "stop_reason" in sdkMessage && typeof sdkMessage.stop_reason !== "undefined"
            ...
            : undefined,
        hasResultText:
          "result" in sdkMessage && typeof sdkMessage.result === "string" ? sdkMessage.result.length > 0 : false,
      });
...
ipcMain.handle("config-file:write", async (_event, payload?: { config?: unknown }) => {
  ...
  logInfo(`[config-file:write] mcp httpServers=${config.mcp.httpServers.length}`);
...
ipcMain.handle(
  ...
  async (_event, payload?: { name?: string; url?: string; headers?: Record<string, string> }) => {
    logInfo(`[mcp-server:test-connection] 开始测试 name=${payload?.name || "(unnamed)"} url=${payload?.url || ""}`);
    const result = await testMcpHttpConnection(payload);
    logInfo(
      `[mcp-server:test-connection] 完成 ok=${result.ok} reachable=${result.reachable} status=${String(result.status)}`
    );
    return result;
  }
);
